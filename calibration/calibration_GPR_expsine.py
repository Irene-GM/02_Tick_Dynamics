...
print("\nGAUSSIAN PROCESS CALIBRATION (Exponential-sine kernel)")
print("-" * 50, "\n")
...
list_periodicity = [1, 5, 10]
list_length_scale_bounds = [(1e-1, 1.0), (1e-1, 5.0)]
list_periodicity_bounds = [(1e-5, 1e2)]

for l in list_length_scale:
    for b in list_length_scale_bounds:
        for p in list_periodicity:
            for pb in list_periodicity_bounds:
                print("")
                print("GPR ExpSineSquared with parameters: ", l, b, p, pb)
                gp_kernel = (1.0 * ExpSineSquared(length_scale=l, length_scale_bounds=b, periodicity=p, periodicity_bounds=pb)) + WhiteKernel(noise_level=1e2)
