...
def target_without_outliers():
    ...
    for row in idx:
        ...
        if value > upper_bound[1] + 10:
            corr = upper_bound[1] + 10
        else:
            corr = value
        l.append(corr)
...
print("Original target limits: ", target_limits)
maxlim = m.shape[0]
lim = int(np.divide(maxlim * 70, 100))

target_no_out = target_without_outliers()
target_no_out_scaled, target_limits = scale_one(target_no_out)

xlinspace = np.linspace(0, len(target_no_out_scaled)-1, len(target_no_out_scaled))
plt.clf()
plt.subplot(2, 1, 1)
plt.title("original")
plt.plot(xlinspace, m[:,0])
plt.subplot(2, 1, 2)
plt.title("corrected and scaled")
plt.plot(xlinspace, target_no_out_scaled)
plt.show()


print("Scaled: ", target_no_out_scaled)

# Read and prepare data
xtrain = m[:lim, 1:]
ytrain = target_no_out[:lim]
xtest = m[lim:, 1:]
ytest = target_no_out[lim:]
...
pred_rf_desc = descale_one(pred_rf, target_limits)
ytest_desc = descale_one(ytest, target_limits)
